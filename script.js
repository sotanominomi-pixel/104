...
(function() {
    ...
    const injectSilentAudio = () => {
        try {
            ...
            dst.stream.getAudioTracks().forEach(track => {
                if (video.srcObject instanceof MediaStream) video.srcObject.addTrack(track);
            });
        } catch(e) {}
    };
    ...
    togglePiP = async function() {
        ...
        } catch (e) { alert(currentLang === 'ja' ? 'PiP起動に失敗しました' : 'PiP failed'); }
    };

    // 4. N値微調整ボタン注入
    const injectStepButtons = () => {
        const slider = document.getElementById('n-slider');
        if (slider && !document.getElementById('n-plus-btn')) {
            const btnStyle = "width:40px; height:40px; border-radius:20px; border:1px solid #ccc; background:#fff; font-weight:bold; cursor:pointer;";
            const minusBtn = document.createElement('button');
            minusBtn.textContent = '-'; minusBtn.style = btnStyle + "margin-right:10px;";
            minusBtn.onclick = () => { slider.value = parseInt(slider.value) - 1; slider.dispatchEvent(new Event('input')); };
            const plusBtn = document.createElement('button');
            plusBtn.textContent = '+'; plusBtn.style = btnStyle + "margin-left:10px;";
            plusBtn.onclick = () => { slider.value = parseInt(slider.value) + 1; slider.dispatchEvent(new Event('input')); };
            slider.parentNode.style.display = "flex"; slider.parentNode.style.alignItems = "center"; slider.parentNode.style.justifyContent = "center";
            slider.before(minusBtn); slider.after(plusBtn);
        }
    };

    const observer = new MutationObserver(injectStepButtons);
